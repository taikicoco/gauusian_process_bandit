...
def make_parameter_analysis(df, f_max, f_sigma, algorithm):
    ...
    param_combinations = [('noise', 'alpha'), ('alpha', 'beta'), ('alpha', 'noise')]
    
    fig, axs = plt.subplots(1, len(param_combinations), figsize=(5*len(param_combinations), 5))
    
    for ax, (x_col, y_col) in zip(axs, param_combinations):
        make_loss_contour_plot(fig, ax, df, x_col, y_col, lev, t_t_r, f_max, f_sigma)
